...
def detect_tree(image_file):
    try:
        ...
        model = get_model()
        t1 = time.time()
        print(f"[TIME] Model access time: {t1 - t0:.4f}s")
        
        # Load and preprocess image
        img = Image.open(image_file).convert('RGB')
        img = img.resize((224, 224))
        t2 = time.time()
        print(f"[TIME] Image resize time: {t2 - t1:.4f}s")
        ...
        t3 = time.time()
        print(f"[TIME] Prediction time: {t3 - t2:.4f}s")
        ...
        tree_keywords = [
            ...
            'vegetable', 'fruit', 'crop', 'agriculture', 'field', 'plantation', 'soil', 'earth',
            'pot', 'backyard' # Added for better detection in urban settings
        ]
        ...
        # If still no tree found, use the top detected object's confidence
        # This prevents "0%" output, but is_tree remains False
        if not is_tree:
            confidence = float(top_pred_score)
